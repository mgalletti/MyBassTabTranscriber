...
from my_bass_tab_transcriber.validators.validator_factory import (
    ValidatorFactory,
    ValidatorTypes,
)
from my_bass_tab_transcriber.services.file_converters import WavAudioFileConverter
from my_bass_tab_transcriber.utils.audio_enhancements import AudioFileQualityImprover
from my_bass_tab_transcriber.models.audio_time_series import AudioTimeSeries
...
class AudioFileService:
    ...
    def save_as_wav(self, output_file: str, sample_rate: int = 16000):
        logger.info(
            f"Converting file '{self.audio_file_path}' and saving it to '{output_file}."
        )
        self.wav_converter.convert_to_wav(
            self.audio_file_path, output_file, sample_rate
        )

    def check_audio_loudness(
        self, lower_threshold: float = 0.05, upper_threshold: float = 0.5
    ):
        logger.info(
            f"Checking loudness of file '{self.audio_file_path}'.",
            extra={
        ...
        if avg_rms < lower_threshold:
            logger.exception(
                "Audio file is too silent. Please provide a louder audio file"
            )
            raise ValueError(
                f"Audio file is too silent. Please provide a louder audio file. Current RMS: {avg_rms}"
            )
        elif avg_rms > upper_threshold:
            logger.exception(
                "Audio file is too loud. Please provide a quieter audio file."
            )
            raise ValueError(
                f"Audio file is too loud. Please provide a quieter audio file. Current RMS: {avg_rms}"
            )
...
class AudioSamplingService:
    @staticmethod
    def sample(
        audio_time_series: AudioTimeSeries, target_sample_rate: int = 44100
    ) -> AudioTimeSeries:
        resampled_audio = librosa.resample(
            audio_time_series.audio,
            orig_sr=audio_time_series.sample_rate,
    ...
    @staticmethod
    def normalize(
        audio_time_series: AudioTimeSeries,
        normalization_strategy: AudioNormalization = AudioNormalization.PEAK,
    ) -> AudioTimeSeries:
        pass
